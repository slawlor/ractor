...
fn convert_messaging_error<TMessage: Message>(
    ...
    match e {
        MessagingErr::SendErr(m) => {
            MessagingErr::SendErr(TMessage::decode(LocalOrSerialized::Serialized(m)).unwrap())
        }
...
fn convert_muxed_message_to_messaging_error<TMessage: Message>(
    ...
    match e {
        MuxedMessage::Message(m) => MessagingErr::SendErr(TMessage::decode(m).unwrap()),
        _ => panic!("Expected a boxed message but got a drain message"),
    }
}
#[cfg(test)]
